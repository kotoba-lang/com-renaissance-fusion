(ns renaissance_fusion.main-test
  "Contract + behavioral test for the renaissance_fusion-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "renaissance_fusion-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'renaissance_fusion.main-test)]
